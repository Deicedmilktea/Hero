 ...
 * @param ins 需要重置的实例
 */
static void CANCommResetRx(CANComm_Instance *ins)
{
    // 当前已经收到的buffer清零
...
{
    CANComm_Instance *comm = (CANComm_Instance *)cancomm;
    CANCommResetRx(comm);
}

CANComm_Instance *CANCommInit(CANComm_Init_Config_s *comm_config)
{
    CANComm_Instance *ins = (CANComm_Instance *)malloc(sizeof(CANComm_Instance));
    memset(ins, 0, sizeof(CANComm_Instance));
...
void CANCommSend(CANComm_Instance *instance, uint8_t *data)
{
    static uint8_t crc8;
...
void *CANCommGet(CANComm_Instance *instance)
{
    instance->update_flag = 0; // 读取后将更新flag置为0
    return instance->unpacked_recv_data;
}

uint8_t CANCommIsOnline(CANComm_Instance *instance)
{
